...
public class LootSelectActionConverter : SelectActionConverterBase<LootSelectActionContext>
{
    ...
    private IReadOnlyDictionary<string, int> LootItems(LootSelectActionContext lootContext) {
        ...
        if (lootContext.SelectionRunnerData is LootSelectionRunnerData lootData) {
            ...
            Debug.LogError($"<color=red>item dict is not type of ({typeof(LootSelectionRunnerData)})</color>");
            return null;
        }
    }

    //private Dictionary<string, int> GetDefaultLootItems(LootSelectionRunnerData lootSelectionData) {
    ...
    //    LootSelectionRunnerData lootSelectionData,
